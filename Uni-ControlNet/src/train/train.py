...
parser.add_argument('--config-path', type=str, default='./configs/global_v21_thingseeg.yaml')
parser.add_argument('--learning-rate', type=float, default=5e-5)
parser.add_argument('--batch-size', type=int, default=2)
parser.add_argument('--training-steps', type=int, default=33080*15)
parser.add_argument('--resume-path', type=str, default='./ckpt/init_global.ckpt')
parser.add_argument('--logdir', type=str, default='./log_thingseeg_ATMSsub8_str2cfg9/')
...
def main():
    ...
    #val_dataset = instantiate_from_config(config['data_val'])
    #val_dataloader = DataLoader(val_dataset, num_workers=num_workers, batch_size=batch_size, pin_memory=True, shuffle=False)

    logger = ImageLogger(batch_frequency=logger_freq, log_images_kwargs={'N': 2})
