...
class GlobalAdapter(nn.Module):
    def __init__(self, in_dim, channel_mult=[2, 8]):
        super().__init__()
        ...
        self.ff2 = FeedForward(dim_out1, dim_out=dim_out2, mult=mult2, glu=True, dropout=0.1)
        self.norm1 = nn.LayerNorm(in_dim)
        self.norm2 = nn.LayerNorm(dim_out1)

    def forward(self, x):
        #print("x shape before GlobalAdapter:", x.shape)
        x = self.ff1(self.norm1(x))
        x = self.ff2(self.norm2(x))
        x = rearrange(x, 'b (n d) -> b n d', n=self.channel_mult[-1], d=self.in_dim).contiguous()
        #print("x shape after GlobalAdapter:", x.shape)
        return x