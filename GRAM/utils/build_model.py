...
def build_model(args):
    ...
    if args.run_cfg.checkpoint:
        if not os.path.exists(args.run_cfg.checkpoint):
            # It's probably in the other output directory
             # Get the current file path and directory
            current_file_path = os.path.abspath(__file__)
            current_dir = os.path.dirname(current_file_path)

            # Go up 3 levels from the current directory
            base_dir = os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))
            path_to_append = base_dir + args.run_cfg.checkpoint
            checkpoint_path = os.path.join(base_dir, args.run_cfg.checkpoint)
            sys.path.append(base_dir+args.run_cfg.checkpoint)
            print(f"Loading checkpoint from {path_to_append}")
            checkpoint = torch.load(path_to_append, map_location='cpu')
        else:
    ...
    #checkpoint = {
    #    (k.replace('module.', '') if not k.startswith("eeg_encoder") 
    #    else (k.replace('eeg_encoder.', 'eeg_encoder.module.') if 'module.' not in k else k)): v 
    ...
    meta_params = [name for name, param in model.named_parameters() 
               if hasattr(param, 'is_meta') and param.is_meta]
    if meta_params:
        LOGGER.warning(f"Meta tensors found before .to(device): {meta_params}")
        raise ValueError("Model contains meta tensors. Initialization failed.")
