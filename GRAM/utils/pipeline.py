...
def train(model, optimizer, train_loader, val_loaders, args, start_step=0, verbose_time=False):
    ...
    print(f"Evaluating every {run_cfg.valid_steps} steps")
    for step, (name, batch) in enumerate(train_loader):
      
        ndata = train_loader.ndata
        task = name.split('--')[0]

        # Add memory cleanup every N steps
        #if step % 399 == 0: #epoch=100 -> 1239 or 199
...
#            LOGGER.info(f"Contra Temp: {model.contra_temp.item()}")
            ...
            if dist.get_rank() == 0:
                ...
                LOGGER.info(f"\n==== saving model at step {global_step} ====")
                model_saver.save(model, global_step, optimizer,best_indicator, run_cfg.save_best)
                # saving eeg encoder model as well
                # model_saver.save(model.eeg_encoder, global_step, optimizer,best_indicator, run_cfg.save_best, extra_prefix = "eeg")
