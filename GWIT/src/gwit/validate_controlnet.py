...
def get_caption_from_classifier(eeg, labels):
    print("Gettin caption from classifier...\n")
...
def generate(data, num_samples=10, limit=4, start=0, classes_to_find=None, single_image_for_eval=False, 
    ...
    for i in tqdm(range(start, num_samples+start)):
        ...
        # added by me, in case i make it work have to remove line 106
        #print("generate fun: ", data[i]['conditioning_image'])
        print("generate fun: ", data[i]['conditioning_image'].shape)
        print("generate fun: ", data[i]['processed_eeg'].shape)
        #control_image = data[i]['conditioning_image'].unsqueeze(0).to(torch.float16) #eeg DEVE essere #,128,512
        control_image = data[i]['processed_eeg'].to(torch.float16)#.unsqueeze(0).to(torch.float16) 
        ...
        if not single_image_for_eval:
            ...
            for j,image in enumerate(images):
                ...
                print(data[i]['image'].dtype)
                transforms.ToPILImage()(data[i]['image'].to(torch.uint8)).resize((512,512)).save(f"{controlnet_path}/ground_truth/gt_{i}_{j}_{label}.png")
    return all_samples
...
print(dset_name)
data_test = load_dataset(dset_name, split="test", cache_dir= base_dir+"\\cache\\").with_format(type='torch') #, cache_dir="/leonardo_scratch/fast/IscrC_GenOpt/luigi/" if "TVIZ" in args.controlnet_path else None).with_format(type='torch')
...
def process_eeg(example):
    #print("base cond image : ", example["conditioning_image"].dtype, example["conditioning_image"].shape)
    #print("base subj: ", example["subject"].dtype, example["subject"].shape)
    conditioning_image = example["conditioning_image"].unsqueeze(0).to(torch.float16) # TODO make it change data_test
    subjects = example["subject"].unsqueeze(0)
    #print("trans cond image 2: ", conditioning_image.dtype, conditioning_image.shape)
    #print("trans subj 2 : ", subjects.dtype, subjects.shape)
    #example["conditioning_image"] = controlnet.controlnet_cond_embedding(conditioning_image, subjects)
    example["processed_eeg"] = controlnet.controlnet_cond_embedding(conditioning_image, subjects)
    return example

data_test = data_test.map(process_eeg)
print(data_test[0]["processed_eeg"].shape, data_test[0]["processed_eeg"].dtype)  
#for i in range(0, len(data_test)):
    #print("base cond image : ", data_test[i]["conditioning_image"].dtype, data_test[i]["conditioning_image"].shape)
    #print("base subj: ", data_test[i]["subject"].dtype, data_test[i]["subject"].shape)
    #conditioning_image = data_test[i]["conditioning_image"].unsqueeze(0).to(torch.float16) # TODO make it change data_test
    #subjects = data_test[i]["subject"].unsqueeze(0)
    #print("trans cond image 2: ", conditioning_image.dtype, conditioning_image.shape)
    #print("trans subj 2 : ", subjects.dtype, subjects.shape)
    #data_test[i]["conditioning_image"] = controlnet.controlnet_cond_embedding(conditioning_image, subjects)
    #print("base transformed cond image: ", data_test[i]["conditioning_image"].shape)
