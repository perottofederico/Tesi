...
def log_validation(
    ...
    for i in range(0, 10):
        ...
        validation_image = controlnet.controlnet_cond_embedding(validation_image, subjects)
...
def make_train_dataset(args, tokenizer, accelerator):
    ...
    def preprocess_train(train_split):
        ...
        train_split["input_ids"] = tokenize_captions(train_split) # Tokenize the captions we have generated
        return train_split
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                #print("\n Pooled Latents: ", pooled_latents.shape)
                pooled_captions = encoder_hidden_states[:,0] #should be [b, 1024]
                #print("\n Pooled Captions: ", pooled_captions.shape)
                
                eeg_embedding= controlnet.controlnet_cond_embedding(controlnet_image, batch["eeg_subjects"])
                #print("\n EEG Embedding: ", eeg_embedding.shape)
                pooled_eeg_embedding = eeg_embedding[:,:,0] 
                pooled_eeg_embedding = torch.mean(pooled_eeg_embedding, dim=1) #should be [b,128]
                #print("\n Pooled Controlnet Image: ", pooled_eeg_embedding.shape)

                #Projection
                projected_latents = latents_proj(pooled_latents).to(accelerator.device) #should be [b,128]
                ...
                #print("\n Projected Latents: ", projected_latents.shape)
                #print("\n Projected Captions: ", projected_captions.shape)
                #print("\n Projected Controlnet Image: ", projected_controlnet_image.shape)
                ...
                # volume computation
                gram_volume = volume_computation3(projected_captions, projected_latents, projected_controlnet_image)
                #print("\n Volume: ", gram_volume.mean())
                ### --------- ###
                ...
                ### --------- ###
                total_loss = loss + 1*gram_volume.mean() # 1 for now, make it an arg TODO
                ### --------- ###
            ...
            logs = {"loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "volume": gram_volume.mean().detach().item(), "total_loss": total_loss.detach().item()}
